def updateInput(inputQueue, queues, inputRunning, time, endedProcess, outputQueue): # to update inputQueue[] i.e. to check if any process has done it's work on input devices or if not than decrease it's time by 1.
	if(len(inputQueue)<=0):
		...
		if(inputQueue[0]['execution'][1]=="0"):
			...
			if(len(inputQueue[0]['execution'])>0):
				if(inputQueue[0]['execution'][0]=='P'):
					queues[inputQueue[0]['currentQueue']].processBuffer.append(inputQueue[0])
