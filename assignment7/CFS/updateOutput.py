def updateOutput(outputQueue, processes, outputRunning, time, endedProcess, inputQueue): # to update outputQueue[] i.e. to check if any process has done it's work on output devices or if not than decrease it's time by 1.
		...
		if(outputQueue[0]['execution'][1]=="0"):
			...
			if(len(outputQueue[0]['execution'])>0):
				if(outputQueue[0]['execution'][0]=='P'):
					if(len(processes)>0):
						outputQueue[0]['vruntime']=processes[0]['vruntime']
