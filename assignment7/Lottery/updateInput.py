...
def updateInput(inputQueue, processes, inputRunning, time, endedProcess, outputQueue, tickets, currentShare): # to update inputQueue[] i.e. to check if any process has done it's work on input devices or if not than decrease it's time by 1.
	if(len(inputQueue)<=0):
