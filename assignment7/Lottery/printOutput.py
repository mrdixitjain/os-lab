def printOutput(endedProcess, scheduling):
	...
	for i in endedProcess:
		print("("+str(i['pid'])+", "+str(i['endTime'])+"),", end=" ")
		sumResponse+=i['endTime']
	print(sumResponse/len(endedProcess))

	print("Response Time", end=" -> ")
	sumResponse=0
	for i in endedProcess:
		print("("+str(i['pid'])+", "+str(i['startTime'])+"),", end=" ")
		sumResponse+=i['startTime']
	print(sumResponse/len(endedProcess))

	print("Waiting Time", end=" -> ")
	sumResponse=0
	for i in endedProcess:
		print("("+str(i['pid'])+", "+str(i['endTime']-i['totalBurst'])+"),", end=" ")
		sumResponse+=i['endTime']-i['totalBurst']
	print(sumResponse/len(endedProcess))
	return