...
if __name__ == "__main__":
	...
	while(len(processes) > 0 or len(allocated) > 0 ):#or len(waiting) > 0):
		# check if any allocated process has completed it's time
		n = updateAllocated(allocated, memory, n)

		# n = updateWaiting(allocated, waiting, memory, succReq, totalReq, n)
		i = 0
		while(i<len(processes)):
			if(processes[i].arrivalTime > time):
				break
			totalReq += 1
			a = allocateMemory(processes[i], memory, n)
			n = a[1]
			if(a[0]):
				...
				# print(processes[i].arrivalTime, processes[i].maxTime, processes[i].memSize)
